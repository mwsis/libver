...
Flags/options:
    ...
    --disable-cli
        disables building of the shared libver CLI (by setting BUILD_CLI=OFF)

    --disable-frontends
        disables building of ecosystem frontends such as cargo-libver
        (by setting BUILD_FRONTENDS=OFF)
...
if [ $CliDisabled -eq 0 ]; then CMakeBuildCliFlag="ON" ; else CMakeBuildCliFlag="OFF" ; fi
if [ $FrontendsDisabled -eq 0 ]; then CMakeBuildFrontendsFlag="ON" ; else CMakeBuildFrontendsFlag="OFF" ; fi
...
if [ $MinGW -ne 0 ]; then
  ...
  cmake \
    -DBUILD_CLI:BOOL=$CMakeBuildCliFlag \
    -DBUILD_FRONTENDS:BOOL=$CMakeBuildFrontendsFlag \
